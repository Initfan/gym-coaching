...
export const useAppStore = create<AppState>()(
  persist(
    (set) => ({
      ...
      nutrition: {
        protein: 120, targetProtein: 200,
        carbs: 150, targetCarbs: 280,
        fats: 45, targetFats: 70,
      },
      updateNutrition: (macros) => set((state) => ({ nutrition: { ...state.nutrition, ...macros } })),
      addMacro: (macro, amount) => set((state) => ({
        nutrition: {
          ...state.nutrition,
      ...
      addWeightLog: (weight) => set((state) => ({
        weightLogs: [...state.weightLogs, { date: new Date().toISOString().split('T')[0], weight }]
      })),

      activeProgram: 'Push Intensity & Hypertrophy A',
